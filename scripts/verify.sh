...
case "$health" in
    ...
    *)        bad "runtime healthcheck 是 ${health:-（查不到容器）}" ;;
esac
...
# ---------------------------------------------------------------------------
# 先確認 runtime 真的接在 hermes-net 上。這不是形式檢查：網路沒接上時
# （compose 中途被打斷、或網路是別的專案留下來的殘骸），Docker 就無法發佈
# 9119，宿主機上根本不會有 listener。症狀是 dashboard 完全連不上，但容器
# 全部 healthy。
...
head_ "7. 沒有宿主機資料夾掛進來"
# ---------------------------------------------------------------------------
# 這套 stack 刻意不掛任何使用者資料夾。這一節驗證的是「掛載點根本不存在」，
# 而不是「有政策擋著」—— 前者連拿到 terminal 的 agent 都繞不過去。
#
# 要開放某個資料夾時見 README 的「需要外部資料夾時：讓 hermes 自己裝一個
# MCP filesystem server」：那條路走的是獨立容器 + MCP over HTTP，資料夾依然
# 不會出現在 runtime 裡，所以下面這幾條檢查在啟用之後仍然應該是綠的。

out="$(in_runtime 'test -e /workspace && echo PRESENT || echo ABSENT')"
if printf '%s' "$out" | grep -q ABSENT; then
    ok "runtime 容器內沒有 /workspace"
else
    bad "runtime 容器內看得到 /workspace" \
        "有人把宿主機資料夾掛回 runtime 了 —— terminal 工具就直接讀得到"
fi

# 唯一允許的 bind mount 是唯讀的 GCP 憑證目錄。其他 bind 一律要問清楚。
binds="$(docker inspect -f '{{range .Mounts}}{{if eq .Type "bind"}}{{.Destination}}:rw={{.RW}} {{end}}{{end}}' \
    "hermes-${INSTANCE}-runtime" 2>/dev/null)"
unexpected="$(printf '%s' "$binds" | tr ' ' '\n' | grep -v '^$' | grep -v '^/opt/gcp:rw=false$' || true)"
if [ -z "$unexpected" ]; then
    ok "runtime 沒有非預期的宿主機 bind mount（binds：${binds:-（無）}）"
else
    bad "runtime 有非預期的宿主機 bind mount：$(printf '%s' "$unexpected" | tr '\n' ' ')" \
        "agent 讀寫得到那個目錄。確認是你自己加的，而不是進化產物改出來的"
fi

# controller 也一樣 —— 它有 docker API，掛進去的資料夾等於整台機器的跳板。
binds="$(docker inspect -f '{{range .Mounts}}{{if eq .Type "bind"}}{{.Destination}} {{end}}{{end}}' \
    "hermes-${INSTANCE}-controller" 2>/dev/null)"
if [ -z "$(printf '%s' "$binds" | tr -d '[:space:]')" ]; then
    ok "controller 沒有任何宿主機 bind mount"
else
    bad "controller 掛了宿主機目錄：$binds" "controller 摸得到 docker API，這個組合等於整台機器"
fi

# 沒有 mcp-fs 容器的話，config.yaml 裡就不該留著指向它的設定 —— 留著的話
# hermes 每次啟動都會去連一個不存在的 host，工具清單靜默變空。
out="$(in_runtime 'grep -q "^mcp_servers:" /opt/data/config.yaml 2>/dev/null && echo SET || echo UNSET')"
if printf '%s' "$out" | grep -q UNSET; then
    ok "config.yaml 沒有設定 MCP server（預設值）"
else
    host="$(in_runtime 'sed -n "s|.*url:.*//\([^:/]*\).*|\1|p" /opt/data/config.yaml' | head -1 | tr -d '\r')"
    if [ -n "$host" ] && in_runtime "getent hosts '$host' >/dev/null 2>&1 && echo OK" | grep -q OK; then
        ok "config.yaml 設了 MCP server，且 $host 解析得到"
    else
        bad "config.yaml 設了 MCP server，但 runtime 解析不到 ${host:-（解析不出主機名）}" \
            "那個容器沒起來 —— hermes 會靜默跳過，agent 看不到任何 MCP 工具"
    fi
fi
